...
date_oecd <- sourcoise::sourcoise_meta("oecd.r")$data_date |> 
  as.Date() |> 
  lubridate::stamp_date("téléchargé le 10/4/2025", locale = "fr_FR")()
...
ggpib <- function(pays, variable) {
  ...
  ggplot(oecd |> filter(country %in% !!pays,
    ...
    ofce::ofce_caption(
      source = "OECD Economic Outlook 118 (EO), {date_oecd}", ofce = FALSE, author = TRUE,
...
ofce::margin_download(oecd |> select(year, country, variable, value))
...
-   Une explication "keynésienne" selon laquelle les politiques macroéconomiques menées après la crise financière de 2008 et les conséquences de la crise des dettes souveraines en zone euro qui ont conduit à un sous-investissement privé et public et une sur-épargne du continent conduisant à un accroissement de la position nette extérieur et une réduction relative des dettes publiques plus importante en zone Euro qu'ailleurs [voir @iags2019; @blot2025]. La crise sanitaire de 2020 a pu avoir des conséquences différenciées accentuant des trajectoires divergentes depuis 2008.
...
date_wid <- sourcoise::sourcoise_meta("wid.r")$data_date |> 
  as.Date() |> 
  lubridate::stamp_date("téléchargée le 10/4/2025", locale = "fr_FR")()
...
gg1 <- function(pays, variable = "aptincj992", type = "eurppp",
  ...
  ggplot(dina |> filter(country%in%pays,
    ...
    theme_ofce(
      ...
      strip.text.y = marquee::element_marquee(size=rel(0.7), hjust = 0)) +
    ofce::ofce_caption(
      source = "World Inequality Database (WID), {date_wid}", ofce = FALSE, author = TRUE,
      code = "https://github.com/xtimbeau/decrochage",
...
ofce::tabsetize2(plots, girafy=TRUE, r=1.5)

```

```{r, results="asis"}

margin_download(dina |> select(year, country, percentile,variable, type, revenu))
...
margin_download(evol |> select(year, country, variable, s10s50))
...
full_dec <- full_dec |>
  ...
  mutate(
    ...
    "<b>Décile global {str_remove(decile, 'd')} année {year}</b>
     ...
     Revenu moyen: {fnum(revenu_moy)} euros 2024 ppp"))
...
ggg <- g50 + g5090 + g90100 +
  ofce_caption(
    note="Les échelles de revenus (axes des *y*) sont différentes pour chaque catégorie de revenu, les valeurs annuelles sont affichées en milieu d'année.",
    source="World Inequality Database (WID), {date_wid}")
